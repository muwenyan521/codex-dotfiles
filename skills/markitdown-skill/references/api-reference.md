...
For Python API use you need `markitdown` importable from your project's
environment — `uv tool install` is for the CLI only and won't be visible
to `import markitdown`.

```bash
# Inside a uv project (recommended)
uv add 'markitdown[all]'

# In an active venv (non-project)
uv pip install 'markitdown[all]'
```

For standalone scripts, prefer a PEP 723 inline-metadata header so
`uv run script.py` self-bootstraps — see `scripts/batch-convert.py`.
