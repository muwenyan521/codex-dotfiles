...
# dependencies = ["markitdown[all]", "tqdm"]
# ///
"""
Batch convert documents to Markdown using MarkItDown.

Usage:
    uv run batch-convert.py ./documents ./output
    uv run batch-convert.py ./documents ./output --extensions pdf,docx,pptx
    uv run batch-convert.py ./documents ./output --workers 8
"""
...
try:
    ...
    print(
        "Error: markitdown not importable. Re-run via uv to provision deps "
        "from the PEP 723 header:"
    )
    print("    uv run batch-convert.py <input_dir> <output_dir>")
    print("Or, in a uv project: `uv add 'markitdown[all]' tqdm`.")
    sys.exit(1)
